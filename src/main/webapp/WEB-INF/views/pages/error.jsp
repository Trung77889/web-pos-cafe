...
<c:choose>
    <c:when test="${statusCode == 404}">
        <c:set var="errorTitle" value="${general['general.error.404.title']}"/>
        <c:set var="errorSubtitle" value="${general['general.error.404.subtitle']}"/>
        <c:set var="errorIcon" value="fi fi-rr-search"/>
    </c:when>

    <c:when test="${statusCode == 500}">
        <c:set var="errorTitle" value="${general['general.error.500.title']}"/>
        <c:set var="errorSubtitle" value="${general['general.error.500.subtitle']}"/>
        <c:set var="errorIcon" value="fi fi-rr-bug"/>
    </c:when>

    <c:otherwise>
        <c:set var="errorTitle" value="${general['general.error.unknown.title']}"/>
        <c:set var="errorSubtitle" value="${general['general.error.unknown.subtitle']}"/>
...
<div class="py-12 text-center">
    ...
    <a href="./" class="btn btn-primary px-4">
        ${general["general.button.backToHome"]}
    </a>
    <a href="javascript:history.back()" class="btn btn-outline-secondary px-4 ms-2">
        ${general["general.button.backToPreviousPage"]}
    </a>
</div>